package pers.hll.aigc4chat.common.base.log.logger.impl;
...
public class XLoggerImpl implements XLogger {
    ...
    private static Level strToLevel(@Nullable String level) {
        if (XTools.strEmpty(level)) {
            ...
            return switch (level) {
                case XLogger.LEVEL_OFF -> Level.OFF;
                case XLogger.LEVEL_ERROR -> Level.SEVERE;
                case XLogger.LEVEL_WARNING -> Level.WARNING;
                case XLogger.LEVEL_NOTICE -> Level.INFO;
                default -> Level.ALL;
            };
        }
    }
